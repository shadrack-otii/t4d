...
@section('title', "Edit Face to Face Schedule | $course->name")
                ...
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="card">
                        ...
                        <div class="body">
                            <div class="row clearfix">
                                <div class="col-sm-12">
                                    <form action="{{ route('backoffice.course.physical_class.update', compact('course', 'physical_class')) }}" method="post" id="schedule-form">
                                        ...
                                        <div class="input-daterange input-group" id="bs_datepicker_range_container">
                                            ...
                                            <div class="form-line">
                                                <input type="date" class="form-control" placeholder="Date start..." name="from" value="{{ old('from', $physical_class->from) }}" min="{{ date('Y-m-d') }}">
                                            </div>
                                            <span class="input-group-addon">to</span>
                                            <div class="form-line">
                                                <input type="date" class="form-control" placeholder="Date end..." name="to" value="{{ old('to', $physical_class->to) }}" min="{{ date('Y-m-d') }}">
                                            </div>
                                        </div>
                                        ...
                                        <div class="input-daterange input-group" id="bs_datepicker_range_container">
                                            ...
                                            <div class="form-line">
                                                <input type="date" class="form-control" placeholder="Date end..." name="booking_end" min="{{ date('Y-m-d') }}" value="{{ old('booking_end', $physical_class->booking_end) }}" min="{{ date('Y-m-d') }}">
                                            </div>
                                        </div>
    ...
    <script>
        new Vue({
            ...
            data: () => ({
                ...
                selectedCity: @json( old('city_id', $physical_class->city_id) )
            }),
            ...
            computed: {
                ...
                currencies() {
                    ...
                    return this.selectedCity ? this.cities.find(city => city.id == this.selectedCity).venue.currencies.map(currency => {
                        ...
                        if ( this.selectedCity == @json($physical_class->city_id) ) {

                            let scheduleCurrency = @json($physical_class->currencies).find(({id}) => id == currency.id);
