...
namespace App\Http\Controllers\Certification\Payment;
...
class PaypalController extends Controller
{
    ...
    public function create(Payment $payment)
    {
        $response = $this->createPayment(
            ...
            $payment->amount,
            $payment->currency, 
            route('certification.payment.paypal.execute'),
            route('certification.payment.paypal.cancel', $payment)
        );
        ...
        if ( $response['status'] == 'failed' ) {
            ...
            return redirect()->route('certification.show', $payment->item->certification_id);
        }
    ...
    public function execute(Request $request)
    {
        ...
        return redirect()->route('certification.show', $paypal_payment->item_payment->item->certification_id);
    }
    ...
    public function cancel(Payment $payment)
    {
        ...
        return redirect()->route('certification.show', $payment->item->certification_id)->with('error', 'Your payment has been canceled.');
    }
}
